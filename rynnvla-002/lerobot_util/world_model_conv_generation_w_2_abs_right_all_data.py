...
def process_a2i_data(
    ...
    for task_path in tqdm(task_paths, desc="Processing Tasks"):
        ...
        for trj in tqdm(trj_list, desc=f"  - Trajectories for '{task_name_readable}'", leave=False):
            ...
            imgs_path = os.path.join(trj_path, 'right_image')
            # right_image is not used in this script but path could be defined here if needed
            # imgs_path_w = os.path.join(trj_path, 'right_image')
            
            if not os.path.exists(action_base_path) or not os.path.exists(imgs_path):
                print(f"    Warning: Missing 'abs_action' or 'right_image' in {trj_path}. Skipping.")
                continue
    ...
    output_filename = f'libero_{task_name_for_output}_his_{his}_train_a2i_{resolution}_abs_right_all_data.json'
